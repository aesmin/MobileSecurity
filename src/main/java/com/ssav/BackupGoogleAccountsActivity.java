...
import com.ssav.antitheft.BackupUtils;
import com.ssav.antitheft.Fragments.AdvancedSettingsFragment;
import com.ssav.antitheft.Fragments.BackupAccountsDialogFragment;
...
public class BackupGoogleAccountsActivity extends Activity implements BackupAccountsDialogFragment.NoticeDialogListener{
    private static final String TAG = "OMS";
    ...
    private void createAegisFolder() {    
        Log.i(TAG, "Creating SSMS folder");
        Thread t = new Thread(new Runnable() {
            @Override
            public void run() {
              try {               
                  File body = new File();
                  body.setTitle("SSMS Backup");
                  body.setDescription("Backup stored by SSMS");
    ...
    private static String getAegisFolder() {
        ...
        String folderID = null;
            try {
                request = service.files().list().setQ("mimeType= 'application/vnd.google-apps.folder' and title = 'SSMS Backup' and trashed = false");
            } catch (IOException e) {
                e.printStackTrace();
                return null;
            }
    ...
    private void storeGoogleAccounts(String accountName) {
        ...
        Editor editor = getSharedPreferences(ACCOUNT_PREFS_NAME, MODE_PRIVATE).edit();
